...
import 'package:flutter_uber_app/widgets/customUpdateTextFiled.dart';
import 'package:flutter_uber_app/widgets/registrationInkWell.dart';
  ...
  void update() async {
    ...
    if (name != null) {
      if (name.length < 4) {
        ...
        usersRef.child(userCurrentInfo.id).child("name").set(name);
      }
    }
    if (email != null) {
      usersRef.child(userCurrentInfo.id).child("email").set(email);
    }
    if (phone != null) {
      usersRef.child(userCurrentInfo.id).child("phone").set(phone);
    }
